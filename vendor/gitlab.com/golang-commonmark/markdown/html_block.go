...
func ruleHTMLBlock(s *StateBlock, startLine, endLine int, silent bool) bool {
	...
	switch {
	case pos+2 < max && isLetter(b) && rStartCond1.MatchString(src[pos:]):
		endCond = rEndCond1.MatchString
	...
	case b == '!' && pos+1 < max && isUppercaseLetter(src[pos+1]):
		endCond = func(s string) bool {
			return strings.Contains(s, ">")
		}
	...
	case pos+2 < max && (isLetter(b) || b == '/' && isLetter(src[pos+1])):
		terminator := true
		switch {
		case rStartCond6.MatchString(src[pos:max]):
			// do nothing
		case rStartCond7.MatchString(src[pos:max]):
			terminator = false
		default:
			return false
		}
